...
const HttpMethod = ziez.HttpMethod;
...
fn setAllowMethods(res: *Response, methods: []const HttpMethod) void {
...
fn isMethodAllowed(requested_method: []const u8, methods: []const HttpMethod) bool {
...
fn methodName(method: HttpMethod) []const u8 {
...
fn middlewareHandler(ptr: ?*anyopaque, req: *Request, res: *Response, next: *ziez.Next) void {
    ...
    const proceed = handle(req, res, config.*);
    if (proceed) next.call();
}

fn middlewareDeinit(ptr: ?*anyopaque, _: std.mem.Allocator) void {
    if (ptr) |p| std.heap.page_allocator.destroy(@as(*CorsConfig, @ptrCast(@alignCast(p))));
}

/// Returns a type-erased Middleware.
/// Config is copied into a heap-owned CorsConfig; freed at app.deinit().
pub fn asMiddleware(config: CorsConfig) ziez.Middleware {
    const cfg = std.heap.page_allocator.create(CorsConfig) catch @panic("ziez-cors: OOM");
    cfg.* = config;
    return .{
        .ptr = cfg,
